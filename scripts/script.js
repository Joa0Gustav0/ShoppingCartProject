  ...
  function organizeProductsByCategories() {
    for (let i = 1; i < storeProductsList.childNodes.length - 1; i++) {
      if (currentButton.getAttribute("name").toString() == "flower-categorie") {
        if (
          storeProductsList.childNodes[i].classList.contains(
            "product-flower-type"
          )
        ) {
          storeProductsList.childNodes[i].style.display = "flex";
          storeProductsList.childNodes[i].classList.add(
            "current-categorie-selected"
          );
        } else {
          ...
          if (
            ...
            storeProductsList.childNodes[i].classList.remove(
              "current-categorie-selected"
            );
          }
        }
...
//PRODUCT INFO TAB
let productPictureSpace = document.querySelector(
  ".selected-product-picture > img"
);
...
let productDescriptionSpace = document.querySelector(
  ".selected-product-description"
);
let totalPrice = document.querySelector(".selected-product-total-price");

//change selected product quantity
let quantityNumber = document.querySelector(
  ".selected-product-price-quantity__quantity > h1"
);
let quantity = 0;
...
function selectProduct(selectedProduct) {
  let productPicture = document.querySelector(
    `#${selectedProduct.id} .product__picture`
  );
  let productName = document.querySelector(
    `#${selectedProduct.id} .product__name`
  );
  let productPrice = document.querySelector(
    `#${selectedProduct.id} .product__price`
  );
  let productDescription = document.querySelector(
    `#${selectedProduct.id} .product__description`
  );
...
let productCartContainerModel = document.querySelector(
  ".default-cart-added-product-container"
);
let cartSubTotalValue = document.querySelector(
  ".cart-shopping-tab__total-price-container__products-price__subtotal"
);
let cartShippingValue = document.querySelector(
  ".cart-shopping-tab__total-price-container__products-price__shipping"
);
let cartDiscountValue = document.querySelector(
  ".cart-shopping-tab__total-price-container__products-price__discount"
);
...
function detectPresenceAtCart() {
  if (
    cartProductsList.childNodes.length > 1 &&
    String(cartProductsList.childNodes[0].innerHTML) == "The cart is empty..."
  ) {
...
function addProductToCart() {
  if (productNameSpace == "---" || Number(quantityNumber.innerText) <= 0) {
    window.alert(
      "Please, select a product adding a minimum quantity of one unit of the product."
    );
  } else {
    ...
    for (
      let i = 0;
      i < cartProductsList.childNodes.length && foundSameProduct == false;
      i++
    ) {
      ...
      for (let i = 0; i < productNameSpace.innerText.length; i++) {
        if (productNameSpace.innerText[i] != " ") {
          preparedClassName =
            preparedClassName + productNameSpace.innerText[i].toString();
        }
        if (productNameSpace.innerText[i] == " ") {
      ...
      if (
        cartProductsList.childNodes[i] != undefined &&
        cartProductsList.childNodes[i].classList.contains(
          `${preparedClassName}`
        )
      ) {
    ...
    if (foundSameProduct == false) {
      let productCartContainerPicture = document.querySelector(
        `.${productCartContainer.className}:last-child img`
      );
      let productCartContainerName = document.querySelector(
        `.${productCartContainer.className}:last-child .cart-added-product-container__product-name`
      );
      let productCartContainerPrice = document.querySelector(
        `.${productCartContainer.className}:last-child .cart-added-product-container__product-price-quantity > h1`
      );
      let productCartContainerQuantity = document.querySelector(
        `.${productCartContainer.className}:last-child .cart-added-product-container__product-price-quantity__quantity > h1`
      );
      ...
      cartProductsList.childNodes[
        cartProductsList.childNodes.length - 1
      ].classList.add(`${preparedClassName}`);
      cartProductsList.childNodes[
        cartProductsList.childNodes.length - 1
      ].setAttribute("id", `${productPriceSpace.innerText}`);
      let productId = cartProductsList.childNodes[
        cartProductsList.childNodes.length - 1
      ]
        .getAttribute("id")
        .toString();
      ...
      cartProductsList.childNodes[
        cartProductsList.childNodes.length - 1
      ].setAttribute("id", `${productIdNumber}`);
    ...
    if (foundSameProduct == true) {
      ...
      for (let i = 0; i < productNameSpace.innerText.length; i++) {
        if (productNameSpace.innerText[i] != " ") {
          preparedName =
            preparedName + productNameSpace.innerText[i].toString();
        }
        if (productNameSpace.innerText[i] == " ") {
      ...
      let productCartContainerPrice = document.querySelector(
        `.${preparedName} .cart-added-product-container__product-price-quantity > h1`
      );
      let productCartContainerQuantity = document.querySelector(
        `.${preparedName} .cart-added-product-container__product-price-quantity__quantity > h1`
      );

      function calculateValuesForCart() {
        productCartContainerQuantity.innerText =
          Number(quantityNumber.innerText) +
          Number(productCartContainerQuantity.innerText);
        ...
        for (let i = 1; i < productCartContainerPrice.innerText.length; i++) {
          alreadyOnCartPrice =
            alreadyOnCartPrice + productCartContainerPrice.innerText[i];
        }
        productCartContainerPrice.innerText = `$${(
          Number(totalPriceValue) + Number(alreadyOnCartPrice)
        ).toFixed(2)}`;
      }
...
function changeQuantityAtCart(quantityPressedButton) {
  ...
  let cartProductsPrice =
    quantityPressedButton.parentNode.parentNode.childNodes[0];

  function recalculateValues() {
    let currentProductInitialValue =
      quantityPressedButton.parentNode.parentNode.parentNode.parentNode.parentNode.getAttribute(
        "id"
      );
    cartProductsPrice.innerText = `$${(
      Number(cartProductsQuantity.innerText) *
      Number(currentProductInitialValue)
    ).toFixed(2)}`;
    calculateCartEndValues();
  }
  if (quantityPressedButton.name == "remove-sharp") {
    if (Number(cartProductsQuantity.innerText) > 1) {
      cartProductsQuantity.innerText =
        Number(cartProductsQuantity.innerText) - 1;
    }
  }
...
function deleteThisCartItem(deleteButtonOfCurrentProduct) {
  ...
  if (cartProductsList.childNodes.length == 0) {
    cartProductsList.innerHTML =
      '<h1 class="cart-empty-sign">The cart is empty...</h1>';
...
const availablePromoCodes = [
  { code: "GARDENIAS100", discount: 100 },
  { code: "GARDENING25", discount: 25 },
  { code: "FLOWERFUL15", discount: 15 },
];
function SetPromoCode() {
  ...
  if (promoCodeInput.value.toUpperCase() != "") {
    ...
    if (promoCodeIsAvailable == true) {
      ...
      window.alert(
        `Congratulations! A ${availablePromoCodes[availablePromoCodeIndex].discount}% discount promo was applied to your cart.`
      );
    }
...
function calculateCartEndValues() {
  ...
  for (let ia = 0; ia < cartProductsList.childNodes.length; ia++) {
    if (cartProductsList.childNodes[ia].childNodes[0].checked) {
      ...
      let productValue =
        cartProductsList.childNodes[ia].childNodes[1].childNodes[1]
          .childNodes[1].childNodes[0];
      for (let ib = 1; ib < productValue.innerText.length; ib++) {
        intCurrentProductValue =
          intCurrentProductValue + productValue.innerText[ib];
      }
...
function calculatePreCheckoutValues(totalQuantity) {
  ...
  for (let i = 1; i < cartSubTotalValue.innerText.length; i++) {
    treatedcartSubTotalValue =
      treatedcartSubTotalValue + cartSubTotalValue.innerText[i];
  }

  cartShippingValue.innerText = `$${(
    (1 / 100) *
    Number(treatedcartSubTotalValue)
  ).toFixed(2)}`;
  if (cartShippingValue.innerText.length < 6) {
    cartShippingValue.innerText = `$0${(
      (1 / 100) *
      Number(treatedcartSubTotalValue)
    ).toFixed(2)}`;
  }
  ...
  for (let i = 1; i < cartShippingValue.innerText.length; i++) {
    treatedcartShippingValue =
      treatedcartShippingValue + cartShippingValue.innerText[i];
  }

  let treatedCartDiscountValue = "";
  for (let i = 0; i < cartDiscountValue.innerText.length - 1; i++) {
    treatedCartDiscountValue =
      treatedCartDiscountValue + cartDiscountValue.innerText[i];
  }

  if (totalQuantity != null || totalQuantity != undefined) {
    cartPreCheckoutQuantity.innerText = `Total (${totalQuantity} items):`;
  }
  cartPreCheckoutPrice.innerText = `$${(
    Number(treatedcartSubTotalValue) +
    Number(treatedcartShippingValue) -
    (treatedCartDiscountValue / 100) *
      (Number(treatedcartSubTotalValue) + Number(treatedcartShippingValue))
  ).toFixed(2)}`;
  if (cartPreCheckoutPrice.innerText.length < 6) {
    cartPreCheckoutPrice.innerText = `$0${(
      Number(treatedcartSubTotalValue) +
      Number(treatedcartShippingValue) -
      (treatedCartDiscountValue / 100) *
        (Number(treatedcartSubTotalValue) + Number(treatedcartShippingValue))
